...
| [BACKLOG.md](BACKLOG.md) | **Open items from testing, in rounds.** Round 1 (13 items) is worked through — each entry keeps the reporter's wording, the diagnosis and how the fix was verified, so a wrong early guess is visible rather than quietly overwritten. Round 2 (26 items) is recorded and not started; it groups by theme, flags four LIKELY REGRESSIONS from the round-1 work, and ends with a suggested order and the decisions needed before building. Read before starting any of them. Also records that voice input is ruled out. |
