...
# --------------------------------------------------------------------------
# The Co-Pilot is now an AGENT: besides describing the current screen it can
...
.http_post_json <- function(url, bearer, body_json) {
  ...
  if (.is_wasm()) {
    ...
    if (!.ensure_pkg("httr", quietly = TRUE)) return(list(status = -1, body = "httr not installed"))
...
.ask_openai_agent <- function(context, history, user_msg, key, dataset_pool,
  ...
  if (!.ensure_pkg("jsonlite", quietly = TRUE))
    return(list(text = "⚠️ Package 'jsonlite' is required for the AI Co-Pilot.", actions = character(0)))

  sys_prompt <- paste(
    "You are the AI Co-Pilot embedded in EasyAnalysis, a universal scientific-analysis app.",
  ...
  for (round in seq_len(max_rounds)) {
    ...
    if (!is.null(msg$.error)) return(list(text = paste0("⚠️ ", msg$.error), actions = actions))
  ...
  list(text = "⚠️ Reached the analysis step limit without a final answer. Try a more specific request.",
       actions = actions)
}
...
chatUI <- function(id) {
  ...
  tagList(
    tags$style(HTML(paste0("
      ...
      .copilot-panel { position: fixed; bottom: 56px; right: 22px; width: 410px; max-width: 94vw; height: 66vh; min-height: 420px;
      ...
      .copilot-key { background: #f1f8f1; border-bottom: 1px solid #e3e8e3; padding: 10px 12px; display: none; }
      .copilot-key.open { display: block; }
      .copilot-key .form-group { margin-bottom: 6px; }
      .copilot-key small { color: #6c757d; }
      ...
      .copilot-actions { font-size: 11px; color: #6c757d; margin: -6px 0 12px 34px; }
      .copilot-actions code { background: #eef4ee; color: #2e7d32; padding: 1px 5px; border-radius: 4px; }
        ...
        if (m$role == "user") {
          ...
          if (!is.null(m$actions) && length(m$actions))
            bubbles[[length(bubbles) + 1]] <- div(class = "copilot-actions",
              HTML(paste0("⚙ ran ", paste0("<code>", m$actions, "</code>", collapse = " → "))))
        }
      }
