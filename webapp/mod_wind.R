...
windToolsUI <- function(id) {
  ...
  accordion(
    ...
    accordion_panel("Export",
      downloadButton(ns("dl_rose"),  tagList(icon("image"),    " Wind rose (PNG)"),
...
windServer <- function(id, dataset_pool, active_dataset) {
  moduleServer(id, function(input, output, session) {
    ...
    # ---- Downloads ----------------------------------------------------------
    output$dl_rose <- downloadHandler(
      filename = function() paste0("windrose_", Sys.Date(), ".png"),
      content  = function(f) {
        p <- rose_plot()
        if (is.null(p)) { writeLines("No plot yet.", f); return() }
        grDevices::png(f, width=800, height=800, res=120)
        print(p)
        grDevices::dev.off()
      }
    )
